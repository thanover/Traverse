...
import type { Environment, EnvironmentWithSystem, Integration } from "../types";
import { STAGES, SYSTEMS, INTEGRATIONS, FLOWS } from "../data";

function getSystemById(id: string) {
...
export default function TraverseApp() {
  ...
  const center = (id: string) => {
    for (let si = 0; si < SYSTEMS.length; si++) {
      ...
      if (env) {
        ...
        return {
          x: sysW + sti * stageW + oi * cw + cw / 2,
          y: headH + si * rowH + rowH / 2,
        };
      }
    }
    return null;
  ...
  const makePath = (a: string, b: string): string | null => {
    const f = center(a),
      t = center(b);
    if (!f || !t) return null;
          ...
          >
            ...
            >
              ...
              {INTEGRATIONS.map((ig, i) => {
                ...
                const inf = inFlow(ig);
                const hov =
                  hoveredInteg?.from === ig.from &&
                  hoveredInteg?.to === ig.to;
                const sel =
                  selectedEnv !== null &&
                      ...
                      >
                        {envs.length === 0 ? (
                          ...
                          envs.map((env) => {
                            ...
                            const dim =
                              activeFlow !== "all" && !envInFlow(env.id);
                            ...
                            return (
                              <button
                                key={env.id}
                                onClick={() =>
                                  !dim && setSelectedEnv(sel ? null : env)
                                }
                                style={{
                                  width: envs.length > 1 ? 72 : 88,
                                  padding: "10px 0",
                                  borderRadius: 8,
                                  ...
                                  transform: sel
                                    ...
                                    : h
                                      ...
                                      : "scale(1)",
                                  boxShadow: sel
                                    ? `0 4px 16px ${stgColor}20`
                                    : h
                                      ? `0 2px 8px ${stgColor}10`
                                      : "none",
                                  opacity: dim ? 0.2 : 1,
                                  position: "relative",
                                  zIndex: sel ? 12 : h ? 10 : 1,
                                }}
                              >
                                <span
                                  style={{
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...
                            fontWeight: 500,
                            color:
                              activeFlow === f.id ? f.color : c.textMuted,
                          }}
                        >
                          {f.label}
                        </span>
